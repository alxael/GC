...
// ====================================
// 
//	Program ce deseneaza o sfera pe care o poate survola;
// 
// 
// 
//	Biblioteci
...
{
	// varfurile 
	// (4) Matricele pentru varfuri, culori, coordonate de texturare, indici
	glm::vec4 Vertices[NR_VF];
	glm::vec3 Colors[NR_VF];
	glm::vec2 TexCoords[NR_VF];
	GLushort Indices[6 * NR_VF]; // pentru fiecare varf: 4 indici ptr o fata, 2 ptr meridian, 2 ptr paralela
	float u, v, x_vf, y_vf, z_vf;
	int index1, index2, index3, index4;

	for (int merid = 0; merid < NR_MERID + 1; merid++) // ne deplasam pe meridiane, apoi pe paralele
	{
		for (int parr = 0; parr < NR_PARR + 1; parr++)
		{
			// implementarea reprezentarii parametrice 
			// valori pentru u si v
			u = U_MIN + parr * step_u; 
			v = V_MIN + merid * step_v;
			// coordonatele varfului corespunzator lui (u,v)
			x_vf = radius * cosf(u) * cosf(v); 
			y_vf = radius * cosf(u) * sinf(v);
			z_vf = radius * sinf(u);

			// identificator (index) ptr varf; coordonate + culoare + coordonate de texturare
			index = merid * (NR_PARR + 1) + parr;
			Vertices[index] = glm::vec4(x_vf, y_vf, z_vf, 1.0);
			Colors[index] = glm::vec3(0.2f + sinf(u), 0.1f + cosf(v), 0.5f + 0.5 * sinf(u));
			...
			if ((parr + 1) % (NR_PARR + 1) != 0) // varful considerat sa nu fie Polul Nord
			{
				...
				index2 = index + (NR_PARR + 1); // dreapta lui v, pe meridianul urmator
			...
			// indice pentru varful procesat la parcurgerea meridianelor
			Indices[4 * NR_VF + index] = index;

			// indice ptr acelasi varf la parcurgerea paralelelor
			index_aux = parr * (NR_MERID + 1) + merid;
			Indices[5 * NR_VF + index_aux] = index;
		}
	};
	...
	glEnableVertexAttribArray(2); // atributul 2 = coordonate texturare
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), (GLvoid*)(sizeof(Vertices) + sizeof(Colors)));
}
...
void RenderFunction(void)
{
	...
	// desenarea punctelor
	//glPointSize(3.0);
	//glDrawArrays(GL_POINTS, 0, NR_VF);
	...
	for (int merid = 0; merid < NR_MERID; merid++)
	{
		glDrawElements(
			...
			NR_PARR + 1,
			GL_UNSIGNED_SHORT,
			(GLvoid*)((4 * NR_VF + merid * NR_PARR + merid) * sizeof(GLushort)));
	}

	//// o varianta mai simplista pt meridiane, in care apare si un segment Nord-Sud
	//// glDrawElements(GL_LINE_STRIP, NR_VF - 1, GL_UNSIGNED_SHORT, (GLvoid*)((4 * NR_VF ) * sizeof(GLushort)));
	...
	for (int parr = 1; parr < NR_PARR; parr++)
	{
		glDrawElements(
			...
			NR_MERID + 1,
			GL_UNSIGNED_SHORT,
			(GLvoid*)((5 * NR_VF + parr * (NR_MERID +1)) * sizeof(GLushort)));
	}
...
int main(int argc, char* argv[])
{
	...
	glutInit(&argc, argv);
	glutInitDisplayMode(GLUT_RGB | GLUT_DEPTH | GLUT_DOUBLE);						//	Se folosesc 3 buffere pentru desen (unul pentru afisare si unul pentru randare => animatii cursive) si culori RGB + 1 buffer pentru adancime;
